...
awk 'NR > 2 {print}' ../../$file_name >> odata    
...
tail -n $number_atoms $outxyz_name >> odata
$optim >> $output_name
...
echo "MODE 1" > odata
cat ../../Template-odata.min >> odata
tail -n $number_atoms $outxyz_name2 >> odata    
$optim >> $output_name2
...
if [ $repeat = 1 ]; then
   ...
   tail -n $number_atoms ../../$file_name >> odata
   ...
   ((limit1=$limit1+3))
   ((limit2=$limit1+$natm1))
   sed -n "$limit1,$limit2 p" neb.0.xyz >> odata  
   $optim > $output_name3
...
#      rm -f *
   ...
   # Finding out the energy and the number of the image highest in energy
   str_nr=`awk '{print NR,$2}' neb.EofS | LC_ALL=C sort -gk2 | tail -n1 | awk '{print $1}'`
   en_TS=`awk '{print NR,$2}' neb.EofS | LC_ALL=C sort -gk2 | tail -n1 | awk -v OFMT=%12.8f '{print $2}'`

   # Finding out the energy of the image lowest in energy and the 1st energy
   en1st=`head -n1 neb.EofS | awk '{print $2}'`
   enmin=`awk '{print NR,$2}' neb.EofS | LC_ALL=C sort -rgk2 | tail -n1 | awk '{print $2}'`
   # Checking if there is a minima separating these 2 in principle 1st neighbors
   ...
   do
     ...
     #se la differenza è maggiore di 0 
     #echo "$num2 - $num1"
     if [ "$( echo "$num2 - $num1 > -0.01" | bc )" -eq 1 ] ; then
       # se change non è più zero -> ci sono minimi in mezzo perché ha iniziato a crescere dopo che decresceva
       if [ $change -ne 0 ] ; then
         badmin=1
       fi
   ...
   if [ $badmin -eq 1 ]; then
      # If there is, then write a bad energy
      en_TS=1.0
   fi

   # Printing to the new xyz file the Min-TS-Min
   cat ../../$file_name > $outxyz_name2 # Min1
   echo "$number_atoms" >> $outxyz_name2
   ...
   ((limit1=$limit1+3))
   ((limit2=$limit1+$natm1))
   sed -n "$limit1,$limit2 p" neb.0.xyz >> $outxyz_name2
   cat $last_name >> $outxyz_name2 
